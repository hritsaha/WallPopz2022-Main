...
//            }
    ...
    fun getArtistWallList(liveData : MutableLiveData<List<ArtistWallData>>){
        firestore.collection("artistwallpapers").orderBy("uploadTime").addSnapshotListener { value, error ->
            val data = value?.toObjects(ArtistWallData::class.java)
            liveData.postValue(data!!)
        }
    }
